...
<script lang="ts">
    ...
    @Component({
        ...
        methods: {
            ...
            // 내구독 레이어 표시
            async drawMyPlace(key, res) {
	            // 구성품만들기
	            let geoJson = await this.$mapbox.createFeature({
		            prefix: key,
		            content: res
	            });
                ...
                if (key === 'FRIENDS') {
	                ...
	                // 레이어만들기
	                await this.$mapbox.createLayer(geoJson, (layer) => {
		                layer.layout['icon-size'] = 0.3;
                    })
                }

	            this.poiStore[key] = await res.map(item => {
		            let ID = `${key}_${item.poiId}`;
    ...
    export default class Subdescript extends Vue  {
        public async init() {
            ...
            let { lat, lng } = this.$mapbox.maps.getCenter();
            let params = {
                gpsLat: lat,
                gpsLon: lng,
                level: 13,
            ...
            // 구독리스트 가져오기
            let friends = await this.$run('map/GET_SUBSCRIPT').then(res => {
                // 즐겨찾기 멤버 배열 cmn
                let friendStr: string[] = res.interestList.map((i: any) => {
                    return i.friendCmn
                });
            ...
            let friendsLayer = await this.$run('map/GET_MYPLACE', {
                ...params,
                type: 'FRIENDS',
            ...
            // MY 가고싶어요
            let myWishLayers = await this.$run('map/GET_MYPLACE', { ...params, type: 'MYWISH' }).then(res => res);
            await this.drawMyPlace('WISH', myWishLayers)

            // MY 갔다왔어요
            let myReviewLayers = await this.$run('map/GET_MYPLACE', { ...params, type: 'MYCHECK' }).then(res => res);
